...
/// <summary>
/// An immutable reference type that wraps a ReadOnlyMemory of byte buffer.
/// </summary>
public sealed class Octets : IEquatable<Octets>, IEnumerable<byte>
{
    private static readonly Octets _empty = new Octets(ReadOnlyMemory<byte>.Empty);

    /// <summary>
    /// The empty instance of Octets, which contains no data. This instance is shared and 
    /// immutable, so it can be safely reused whenever an empty sequence of bytes is needed.
    /// </summary>
    ...
    /// <summary>
    /// Returns the internal immutable array of buffers.
    /// </summary>
    ...
    /// <summary>
    /// Gets the internal buffers represented as a <see cref="System.Buffers.ReadOnlySequence{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Returns the sum of the lengths of all internal buffers.
    /// </summary>
    public int Length => _length;

    /// <summary>
    /// Returns true if the internal buffers contain are empty.
    /// </summary>
    public bool IsEmpty => _length == 0;

    /// <summary>
    /// Returns true if the internal buffers contain at most one buffer.
    /// </summary>
    ...
    /// <summary>
    /// Creates an instance, copying the sources to a single internal buffer.
    /// </summary>
    ...
    /// <summary>
    /// Returns a tuple of three Octets, where the first (head) is of the specified length and the second (rest) contains the remaining bytes.
    /// </summary>
    ...
    /// <summary>
    /// Returns a tuple of two Octets, where the second (tail) is of the specified length, and the first (rest) contains the remaining bytes.
    /// </summary>
    ...
    /// <summary>
    /// Returns a tuple of three Octets, where the first (head) and second (body) are of the specified lengths, and the third (tail) contains the remaining bytes.
    /// </summary>
    /// <param name="headLength"></param>
    ...
    /// <summary>
    /// Returns a tuple of three Octets, where the first (head) and third (tail) are of the specified lengths, and the second (body) contains the remaining bytes.
    /// </summary>
    /// <param name="headLength"></param>
    /// <param name="tailLength"></param>
    /// <returns></returns>
