...
/// <summary>
/// A helper type for working with Guid values in a way that allows for efficient reading 
/// and writing to spans of bytes, with support for both big-endian and little-endian formats. 
/// This struct uses explicit layout to overlay the Guid value with its individual components, 
/// enabling direct access to the parts of the Guid without needing to parse it as a string. 
/// The struct is immutable, and all operations that modify the data return new instances. 
/// It provides methods for converting between Guid and byte representations, making it useful 
/// for scenarios such as serialization, network communication, or low-level data manipulation 
/// where performance is critical.
/// </summary>
[StructLayout(LayoutKind.Explicit, Size = 16)]
public readonly struct GuidHelper : IEquatable<GuidHelper>
{
    [FieldOffset(0)]
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
    ...
    /// <summary>
    /// Creates an instance from a Guid value, allowing access to its individual components through the Part_A to Part_K fields.
    /// </summary>
    ...
    /// <summary>
    /// Reads a Guid from a span of bytes.
    /// </summary>
    /// <param name="source">The span of bytes to read from.</param>
    /// <param name="bigEndian">Indicates whether the byte order is big-endian.</param>
    /// <returns>The Guid value read from the span.</returns>
    ...
    /// <summary>
    /// Writes a Guid to the specified span using the given endianness.
    /// </summary>
    /// <remarks>Writes bytes directly into the target span without allocating. The target span must be at
    /// least 16 bytes; otherwise an exception is thrown.</remarks>
    /// <param name="target">The span to receive the Guid bytes. Must be at least 16 bytes long.</param>
    /// <param name="bigEndian">true to write bytes in big-endian order; false for little-endian order.</param>
    ...
    /// <inheritdoc />
    public bool Equals(GuidHelper other) => GuidValue.Equals(other.GuidValue);

    /// <inheritdoc />
    public override bool Equals(object? obj) => obj is GuidHelper other && Equals(other);

    /// <inheritdoc />
    public override int GetHashCode() => GuidValue.GetHashCode();

    /// <inheritdoc />
    public static bool operator ==(GuidHelper left, GuidHelper right) => left.Equals(right);
